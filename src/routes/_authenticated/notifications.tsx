    ...
    <PortalShell role={role}>
      ...
      {items.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-border p-10 text-center bg-card/50">
          ...
          <p className="mt-1 text-xs text-muted-foreground">Web, Android and iOS push notification architecture is wired and ready to enable in Phase 2.</p>
        </div>
      ) : (
        <ul className="space-y-3">
          {items.map((n) => (
            <li key={n.id} className="rounded-2xl border border-border bg-card p-5 flex gap-4">
